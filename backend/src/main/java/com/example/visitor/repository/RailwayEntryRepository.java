...
public interface RailwayEntryRepository extends JpaRepository<RailwayEntry, Long> {
    ...
    @Query("SELECT e FROM RailwayEntry e WHERE e.timestamp >= :from AND e.timestamp <= :to ORDER BY e.timestamp DESC")
    List<RailwayEntry> findByTimestampBetweenOrderByTimestampDesc(
